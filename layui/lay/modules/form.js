...
layui.define('layer', function(exports){
    ...
    var submit = function(){
        ...
        layui.each(fieldElem, function(_, item){
            ...
            // 修复 input 数组取值问题
            var nameMatch=item.name.match(/^(.+)\[(.*)\]$/);
            if(nameMatch) {
                if(!field[nameMatch[1]]) field[nameMatch[1]] = new Array();
                nameMatch[2]=="" ? field[nameMatch[1]].push(item.value) : field[nameMatch[1]][nameMatch[2]] = item.value;
            } else {
                field[item.name] = item.value;
            }
        });
